...
public class ProductDAOImple implements ProductDAO {
    ...
    public void addProduct(Product p) {
        String sql = "INSERT INTO PRODUCT(ma_loai_san_pham, ten_san_pham, hinh_anh_1, hinh_anh_2, hinh_anh_3, gia_ban, chi_tiet_san_pham, mo_ta) VALUES(?,?,?,?,?,?,?,?);";
        try {
            Connection con = ConnectionDB.getConnectionDB();
            PreparedStatement ps = (PreparedStatement) con.prepareStatement(sql);
            ...
            ps.setInt(6, p.getGia_ban());
            ps.setString(7, p.getChi_tiet_san_pham());
            ps.setString(8, p.getMo_ta());
            ps.executeUpdate();
            con.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void editProduct(Product p) {
        String sql = "UPDATE PRODUCT SET ma_loai_san_pham=?, ten_san_pham=?, hinh_anh_1=?, hinh_anh_2=?, hinh_anh_3=?, gia_ban=?, chi_tiet_san_pham=?, mo_ta = ? WHERE ma_san_pham='" + p.getMa_san_pham() + "';";
        try {
            Connection con = ConnectionDB.getConnectionDB();
            PreparedStatement ps = (PreparedStatement) con.prepareStatement(sql);
            ps.setInt(1, p.getMa_loai_san_pham());
